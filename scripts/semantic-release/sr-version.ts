 ...
 */
export async function version(pluginConfig: SemanticPluginConfig, context: SemanticContext): Promise<void> {
  ...
  try {
    ...
    logger.error(`❌ Failed to update version information: ${(error as Error).message}`)
    throw error
  }
}