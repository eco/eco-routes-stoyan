...
describe('semantic-release assets directory structure', () => {
  it('should maintain the expected directory structure', () => {
    ...
    const actualUtilFiles = fs.readdirSync(utilsDir)
    ...
    const indexContent = fs.readFileSync(path.join(utilsDir, 'index.ts'), 'utf8');
    ...
    const topLevelDirs = fs.readdirSync(assetsDir)
