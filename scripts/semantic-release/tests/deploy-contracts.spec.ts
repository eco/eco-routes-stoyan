...
jest.mock('../deploy-contracts', () => {
  ...
  return {
    ...
    deployRoutesContracts: async (context: SemanticContext, packageName: string) => {
...
describe('deployRoutesContracts', () => {
  ...
  it('should delete the build directory if it exists before deployment', async () => {
    ...
    expect(context.logger.log).toHaveBeenCalledWith('Deleting existing build directory: /test/cwd/build');
    expect(context.logger.log).toHaveBeenCalledWith('Build directory deleted successfully');
  });
  ...
  it('should not attempt to delete the build directory if it does not exist', async () => {
    ...
    expect(context.logger.log).not.toHaveBeenCalledWith('Deleting existing build directory: /test/cwd/build');
    expect(context.logger.log).not.toHaveBeenCalledWith('Build directory deleted successfully');
  });
  ...
  it('should call determineSalts with the correct version', async () => {
    ...
    // Assert
    expect(determineSalts).toHaveBeenCalledWith(
      '1.0.0', 
      context.logger
    );
  });
  ...
  it('should handle errors properly', async () => {
    ...
    // Act & Assert
    await expect(deployRoutesContracts(context, 'test-package')).rejects.toThrow(testError);
    expect(context.logger.error).toHaveBeenCalledWith('❌ Contract deployment failed');
