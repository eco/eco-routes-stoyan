...
describe('Verify Conditions Function', () => {
  ...
  it('should pass verification when all conditions are met', async () => {
    ...
    expect(context.logger.log).toHaveBeenCalledWith('Verifying conditions for eco-routes release...');
    expect(context.logger.log).toHaveBeenCalledWith('Version validation passed');
    expect(context.logger.log).toHaveBeenCalledWith('✅ All conditions verified successfully');
  });
  ...
  it('should skip version check when nextRelease is not provided', async () => {
    ...
    expect(contextWithoutRelease.logger.log).toHaveBeenCalledWith('✅ All conditions verified successfully');
  });
});