...
describe('Semantic Release Helpers', () => {
  ...
  describe('fetchLatestPackageVersion', () => {
    it('should return version info with isNewer flag when package exists', async () => {
      ...
      (pacote.manifest as jest.Mock).mockResolvedValue({ version: publishedVersion });
      (semver.gt as jest.Mock).mockReturnValue(true);
      
      // Act: Call the function
      const result = await fetchLatestPackageVersion(testPackageName, testVersion, mockLogger);
    ...
    it('should handle version parsing errors', async () => {
      ...
      // Act: Call the function
      const result = await fetchLatestPackageVersion(testPackageName, invalidVersion, mockLogger);
  ...
  describe('getPackageInfo', () => {
    it('should read and parse package.json', () => {
      ...
      expect(fs.readFileSync).toHaveBeenCalledWith(`${testDir}/package.json`, 'utf-8');
    });
  });
});