...
jest.mock('../sr-build-package', () => {
  ...
  return {
    ...
    setPublishingPackage: jest.fn().mockImplementation(async (context, packageName) => {
      if (!context?.nextRelease) {
        context.logger.log('No release detected, skipping TypeScript-only package creation');
        return;
      }
...
import { buildPackage, setPublishingPackage, CONTRACT_TYPES } from '../sr-build-package';
...
describe('Semantic Release Build Package', () => {
  ...
  describe('buildPackage function', () => {
    it('should successfully build the package', async () => {
      ...
      expect(context.logger.log).toHaveBeenCalledWith('✅ Package build completed successfully');
    ...
    it('should handle errors during build process', async () => {
      ...
      expect(context.logger.error).toHaveBeenCalledWith('❌ Package build failed');
    });
    ...
    it('should skip build when no release is detected', async () => {
      ...
      expect(contextWithoutRelease.logger.log).toHaveBeenCalledWith('No release detected, skipping package build');
    });
  });
  ...
  describe('setPublishingPackage function', () => {
    it('should update package.json for main package', async () => {
      ...
      expect(context.logger.log).toHaveBeenCalledWith(`Updating package.json for ${PACKAGE.ROUTES_PACKAGE_NAME}`);
    });
    ...
    it('should update package.json for TS-only package', async () => {
      ...
      // Assert: Should log the correct message
      expect(context.logger.log).toHaveBeenCalledWith(`Updating package.json for ${PACKAGE.ROUTES_TS_PACKAGE_NAME}`);
    });
    ...
    it('should skip when no release is detected', async () => {
      ...
      await setPublishingPackage(contextWithoutRelease, PACKAGE.ROUTES_PACKAGE_NAME);
      
      // Assert: Skip message should be logged
      expect(contextWithoutRelease.logger.log).toHaveBeenCalledWith('No release detected, skipping TypeScript-only package creation');
    });
  });
