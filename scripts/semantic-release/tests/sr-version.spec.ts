...
describe('Semantic Release Version Function', () => {
  ...
  beforeEach(() => {
    ...
    (updaterModule.updateSolidityVersions as jest.Mock).mockReturnValue(updatedFilesCount);
  ...
  it('should update versions in Solidity files and package.json', async () => {
    ...
    expect(context.logger.log).toHaveBeenCalledWith(`Updating version information to ${testVersion}`);
    expect(context.logger.log).toHaveBeenCalledWith(`Updated version in ${updatedFilesCount} Solidity files`);
    expect(context.logger.log).toHaveBeenCalledWith(`✅ Version information updated successfully to ${testVersion}`);
  });
  ...
  it('should throw error when package.json update fails', async () => {
    ...
    (updaterModule.updatePackageJsonVersion as jest.Mock).mockImplementationOnce(() => {
      throw new Error(errorMessage);
    });
