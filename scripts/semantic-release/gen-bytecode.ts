...
const CONTRACTS_TO_DEPLOY: Contract[] = [
  ...
  { name: 'Inbox', args: ['0xB963326B9969f841361E6B6605d7304f40f6b414', true, []] },
...
/**
 * Get the contract path for verification based on contract name
 * @param contractName The name of the contract
 * @returns Formatted contract path for verification
 */
...
/**
 * Saves constructor arguments and parameter types for each contract and environment
 * This will be used during verification to properly encode the arguments
 * @param deploymentData Deployment data including contract arguments
 * @param outputPath Path to save the constructor arguments JSON
 */
export function saveConstructorArgs(deploymentData: Record<string, any>, outputPath: string): void {
  // Ensure directory exists
  const directory = path.dirname(outputPath)
  if (!fs.existsSync(directory)) {
    fs.mkdirSync(directory, { recursive: true })
  ...
  // Structure to hold constructor arguments by environment and contract
  const constructorData: Record<string, Record<string, {
    args: any[]
    types: string
    encoded: string
  }>> = {}
  ...
  for (const [envName, envData] of Object.entries(deploymentData)) {
    ...
    constructorData[envName] = {}

    // For each contract in this environment
    for (const [contractName, contractData] of Object.entries(envData.contracts)) {
      if (!contractData) continue

      // Get the raw constructor arguments
      const args = (contractData as any).args || []

      // Determine parameter types for this contract
      const types = getContractParameterTypes(contractName)

      // Pre-encode the constructor arguments
      let encoded = ''
      if (args.length > 0) {
        try {
          encoded = encodeAbiParameters(parseAbiParameters(types), args)
          console.log(`Pre-encoded constructor args for ${contractName}: ${encoded.substring(0, 30)}...`)
        } catch (error) {
          console.error(`Error encoding constructor args for ${contractName}: ${(error as Error).message}`)
        }
      }

      // Store the data
      constructorData[envName][contractName] = {
        args,
        types,
        encoded
      }

      console.log(`Stored constructor data for ${contractName} in environment ${envName}`)
    }
  }

  // Write the constructor data to file
  fs.writeFileSync(outputPath, JSON.stringify(constructorData, null, 2))
  console.log(`Constructor data saved to ${outputPath}`)
}

/**
 * Get parameter types for a specific contract's constructor
 * @param contractName Name of the contract
 * @returns String of comma-separated parameter types
 */
function getContractParameterTypes(contractName: string): string {
  // Define parameter types for known contracts
  switch (contractName) {
    case 'Inbox':
      return 'address, bool, address[]'

    case 'HyperProver':
      return 'address, address'

    case 'IntentSource':
      return 'address, bool, bool'

    // Add more contract definitions as needed

    default:
      console.warn(`No predefined parameter types for ${contractName}, using fallback`)
      return 'address'
  }
}
