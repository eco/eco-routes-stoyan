...
import fs from 'fs'
import { fetchLatestPackageVersion, getPackageInfo, getTypeScriptPackageName, isValidVersion } from "./helpers"
 ...
 */
export async function verifyConditions(pluginConfig: SemanticPluginConfig, context: SemanticContext): Promise<void> {
  ...
  const requiredEnvVars = [ENV_VARS.PRIVATE_KEY, ENV_VARS.ALCHEMY_API_KEY, ENV_VARS.NPM_TOKEN]
  ...
  if (nextRelease) {
    ...
    const packageInfo = await fetchLatestPackageVersion(packageName, version, logger)
