...
export function updateSolidityVersions(
    ...
    files.forEach(file => {
      ...
      if (stat.isDirectory()) {
        ...
        if (versionRegex.test(content)) {
          ...
          const updatedContent = content.replace(versionRegex, newVersionFunction)
  ...
  try {
    ...
    logger.log(`Successfully updated ${updatedCount} Solidity files with version ${version}-${gitHash}`)
    return updatedCount
  } catch (error) {
    logger.error(`Error updating Solidity versions: ${(error as Error).message}`)
    throw error
  }
}
...
export function updatePackageJsonVersion(
  ...
  try {
    ...
    logger.error(`Error updating package.json version: ${(error as Error).message}`)
    throw error
  }
}