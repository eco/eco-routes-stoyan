...
describe('extract-salt', () => {
  ...
  describe('determineSalts function', () => {
    ...
    it('should log appropriate messages during salt calculation', async () => {
      ...
      // Assert
      expect(mockLogger.log).toHaveBeenCalledWith('major/minor version (5.6), calculating salt');
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining('Using salt for production:'));
      expect(mockLogger.log).toHaveBeenCalledWith(expect.stringContaining('Using salt for pre-production:'));
    });
