import { encodeFunctionData, Hex, encodeDeployData, keccak256, toHex, concat, decodeDeployData } from 'viem'
import fs from 'fs'
...
// Address of the CREATE2Deployer contract
const CREATE_X_DEPLOYER_ADDRESS = '0xba5Ed099633D3B313e4D5F7bdc1305d3c28ba5Ed'

// List of contracts to deploy
const CONTRACTS_TO_DEPLOY: Contract[] = [{ name: 'IntentSource', args: [] }, { name: 'Inbox', args: [] }, { name: 'HyperProver', args: [] }]
 ...
 */
export function generateBytecodeDeployData(
  create2Params:
    {
      value: bigint,
      salt: Hex,
  ...
  const contractJsonPath = path.join(process.cwd(), 'out', `${contractName}.sol`, `${contractName}.json`)
...
decodeDeployData({
  ...
  const deployBytecode = encodeFunctionData({
    ...
    args: [uniqueSalt, deploymentBytecode]
  })
  ...
  for (const contract of CONTRACTS_TO_DEPLOY) {
    ...
    try {
      ...
      deploymentData.contracts[contractName] = {
        uniqueSalt: contractData.uniqueSalt,
        keccakedSalt: keccak256(contractData.uniqueSalt),
      ...
      console.error(`Error generating deployment data for ${contractName}:`, error)
    }
  }
